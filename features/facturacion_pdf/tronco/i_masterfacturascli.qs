...
function factPdf_establecerDatosPdf():Array
{
	...
	if (cursor.action() == "i_resfacturascli")
		datosPdf["nombreFichero"] = util.translate("scripts", "Resumen_de_facturas_%1-%2-%3").arg(hoy.getDate()).arg(hoy.getMonth()).arg(hoy.getYear());
	else
		datosPdf["nombreFichero"] = util.translate("scripts", "Detalle_de_facturas_%1-%2-%3").arg(hoy.getDate()).arg(hoy.getMonth()).arg(hoy.getYear());
