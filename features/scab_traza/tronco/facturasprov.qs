...
function scabTraza_cargarArrayLotes():Array
{
	...
	qryLotes.setTablesList("lineasfacturasprov,movilote");
	qryLotes.setSelect("ml.codlote");
	qryLotes.setFrom("lineasfacturasprov lf INNER JOIN movilote ml ON lf.idlinea = ml.idlineafp");
	qryLotes.setWhere("lf.idfactura = " + cursor.valueBuffer("idfactura") + " GROUP BY ml.codlote");
