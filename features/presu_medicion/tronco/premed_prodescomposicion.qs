/***************************************************************************
                      premed_prodescomposicion.qs  -  description
                             -------------------
...
class oficial extends interna {
	...
	function actualizarDescomp( idD:Number ) {
		this.ctx.oficial_actualizarDescomp( idD );
	}
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
class ifaceCtx extends head {
	...
	function pub_actualizarDescomp( idD:Number ) {
		this.actualizarDescomp( idD );
	}
}
...
function interna_init() {
	...
	var where:String = "iddescomp = " +  this.cursor().valueBuffer( "iddescomp" );

	if ( util.sqlSelect( "premed_medides", "COUNT(*)", where ) )
...
function oficial_actualizarDescomp( idD:Number ) {
	var util:FLUtil = new FLUtil();
	...
	if ( cur.next() ) {
		...
		if ( util.sqlSelect( "premed_medides", "COUNT(*)", where ) ) {
			cantidadMedicion = parseFloat( util.sqlSelect( "premed_medides", "SUM(total)", where ) );
			cantidad = parseFloat( util.roundFieldValue( cantidadMedicion, "premed_prodescomposicion", "cantidad" ) );
			cur.setValueBuffer( "cantidad", cantidad );
		 } else
			cantidad = parseFloat( cur.valueBuffer( "cantidad" ) );

		idP = cur.valueBuffer( "idpartida" );
		precio = parseFloat( cur.valueBuffer( "precio" ) );
		coste = parseFloat( util.roundFieldValue( cantidad * precio, "premed_prodescomposicion", "coste" ) );
